...
// Refactor so it uses async/await
const startTime = Date.now();
const allStartStationsPromise = fetch(startLocation)
	.then(res => res.json());

const allEndStationsPromise = fetch(endLocation)
	.then(res => res.json());

Promise.all([allStartStationsPromise, allEndStationsPromise])
	.then(([allStartStations, allEndStations]) => {
		const startStation = firstStation(allStartStations);
		const endStation = firstStation(allEndStations);
